...
**Properties** extend crumbs with custom attributes. Every crumb has a value for every defined property—there is no "not set" state. Built-in properties (priority, type, description, owner, labels) are available out of the box. Applications can define additional properties at runtime without schema migrations. Crumb dependencies use `child_of` links, not a property.
